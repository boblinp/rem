...
static void *l_start(struct seq_file *m, loff_t * pos)
{
	int i;
	 ...
	 struct queue_show * l_s;
		...
		if (array_element_rem > queue_array_count)
		{
			count=1;
			l_s = (struct queue_show *)queue_show_base_rem;
...
static void *l_next(struct seq_file *m, void *p, loff_t * pos)
{
		...
		++queue_array_count;
		++count;
		if ((*pos != 0) && ((queue_array_count >= array_element_rem) || (count>40))) {
                return NULL;
        }
		++l;
...
static int l_show(struct seq_file *m, void *p)
{
        ...
        seq_printf(m, "%u\t%u\t%u\t%lld\t%u\t%lld\t%lld\t%lld\t", 
					s->numbers, s->length, s->mark_type, *((long long *)&s->p), s->in ,*((long long *)&s->in_avg), *((long long *)&s->pktsize),  *((long long *)&s->price_link));
		seq_printf(m, "\n");
